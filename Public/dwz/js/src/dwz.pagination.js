...
(function($){
	$.fn.extend({
		pagination: function(opts){
				...
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger()) {
							dwzPageBreak({targetType:pc.targetType(), pageNum:pageNum}, pc.rel());
						}
					});
			...
			function _bindEvent($target, pageNum, targetType, rel){
				$target.bind("click", {pageNum:pageNum}, function(event){
					dwzPageBreak({targetType:targetType, pageNum:event.data.pageNum}, rel);
		...
		orderBy: function(options){
			var op = $.extend({ targetType:"navTab", asc:"asc", desc:"desc"}, options);
			return this.each(function(){
				var $this = $(this).css({cursor:"pointer"}).click(function(){
					...
					dwzPageBreak({targetType:op.targetType, orderField: orderField, orderDirection: orderDirection});
				});
