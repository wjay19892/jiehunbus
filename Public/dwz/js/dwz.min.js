...
if(json.statusCode===undefined&&json.info===undefined){
if(alertMsg)return alertMsg.error(json);
else return alert(json);}
if(json.statusCode==DWZ.statusCode.error){
if(json.info&&alertMsg)alertMsg.error(json.info);}else if(json.statusCode==DWZ.statusCode.timeout){
if(alertMsg)alertMsg.error(json.info||DWZ.msg("sessionTimout"),{okCall:DWZ.loadLogin});
else DWZ.loadLogin();}else{
if(json.info&&alertMsg)alertMsg.correct(json.info);};},
...
alertMsg.error(json.info||DWZ.msg("sessionTimout"),{okCall:function(){
if($.pdialog)$.pdialog.checkTimeout();
if(navTab)navTab.checkTimeout();
DWZ.loadLogin();}});}
if(json.statusCode==DWZ.statusCode.error){
if(json.info)alertMsg.error(json.info);}else{
...
if($this.isTag("table")){
$this.removeAttr("layoutH").wrap('<div layoutH="'+iLayoutH+'" style="overflow:auto;height:'+iH+'"></div>');}else{
...
$tr.click(function(event){
   var jvf_selected = $trs.filter(".selected");
   var rel = new Array();
   var rel_str = '';
if (event.ctrlKey) {
	$tr.toggleClass("selected");
}else if(event.shiftKey && jvf_selected.html() != null){
	var jvf_selected_index = $trs.index(jvf_selected);
	var jvf_tr_index = $trs.index($tr);
	if(jvf_selected.length == 1 && (jvf_selected_index != jvf_tr_index)){
		if(jvf_selected_index < jvf_tr_index){
			jvf_selected.nextUntil($tr).addClass("selected");
		}else{
			jvf_selected.prevUntil($tr).addClass("selected");
		}
	}else{
		jvf_selected.removeClass("selected");
	}
	$tr.addClass("selected");
}else{
	jvf_selected.removeClass("selected");
...
jvf_selected = $trs.filter(".selected");
jvf_selected.each(function(){
...
$box.find("[layoutH]").layoutH();}});}else if(form)navTab.reload($(form).attr("action"),{data:params,callback:args.callback});}
...
$box.find("[layoutH]").layoutH();}});}else if(args.targetType=="dialog")dialogPageBreak(args);
...
html='<input type="hidden" name="'+itemDetail+'[#index#].'+field.lookupName+'.id"/>'+'<input type="text" name="'+lookupFieldName+'"'+suggestFrag+' size="'+field.size+'" class="'+field.fieldClass+'"/>'+'<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+itemDetail+'" lookupName="'+field.lookupName+'" index="#index#" title="查找带回">查找带回</a>';
break;
case 'lookupField':
html='<input type="text" name="'+lookupFieldName+'" size="'+field.size+'" class="'+field.fieldClass+'" readonly="readonly"/>';
break;
case 'attach':
html='<input type="hidden" name="'+itemDetail+'[#index#].'+field.lookupName+'.id"/>'+'<input type="text" name="'+lookupFieldName+'" size="'+field.size+'" readonly="readonly" class='+field.fieldClass+'"/>'+'<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+itemDetail+'" lookupName="'+field.lookupName+'" index="#index#" width="560" height="300" title="查找带回">查找带回</a>';
break;
case 'enum':
$.ajax({
...
break;
case 'date':
html='<input type="text" name="'+fieldName+'" class="date '+field.fieldClass+'" format="'+field.patternDate+'" size="'+field.size+'"/>'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
default:
