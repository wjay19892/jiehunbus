...
$(document).ready(function(){
	talk_aboutDetail(<?php echo ($talk_aboutdata['tid']?$talk_aboutdata['tid']:$talk_aboutdata['id']); ?>);
});
</script>
<!--头部——开始-->
<div class="mainbody body_bot body_con clearfix">
    <div class="jvf_body clearfix">
        <div class="detail_left jvf_fl">
            <div class="jvf_frame ss_detail_left_con clearfix">
                <div class="ss_detail_le jvf_fl clearfix">
                    <div class="ss_userpic jvf_fl">
                        <a class="jvf_getUser" href="<?php echo U('User/space/id/'.$talk_aboutdata['uid']);?>" uid="<?php echo ($talk_aboutdata['uid']); ?>">
                            <img src="<?php echo ($talk_aboutdata["header"]); ?>">
                        </a>
                    </div>
                </div>
                <div class="ss_detail_ri jvf_fr">
                    <div class="ss_detail_ri_con">
                        <a class="ft_bold" href="<?php echo U('User/space/id/'.$talk_aboutdata['uid']);?>">
                            <?php echo ($talk_aboutdata["name"]); ?> ：
                        </a>
                        <?php echo ($talk_aboutdata["content"]); ?>
                        <?php if(is_array($talk_aboutdata["accessory"])): $i = 0; $__LIST__ = $talk_aboutdata["accessory"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><img src="<?php echo ($vo["origin"]); ?>" /><?php endforeach; endif; else: echo "" ;endif; ?>
                    </div>
                    <?php if(!empty($talk_aboutdata["source"])): ?><div class="ss_zb">
                        <a href="<?php echo U('User/space/id/'.$talk_aboutdata['source']['uid']);?>">
                            <?php echo ($talk_aboutdata["source"]["name"]); ?>：
                        </a>
                        <?php echo ($talk_aboutdata["source"]["content"]); ?>
                    </div><?php endif; ?>
                    <div class="user_say_bot clearfix">
                        <div class="jvf_fl">
                            <span title="<?php echo (toDate($talk_aboutdata["addtime"])); ?>">
                                <?php echo (dgmdate($talk_aboutdata["addtime"])); ?>
                            </span>
                        </div>
                        <div class="jvf_fr">
                            <a tid="<?php echo ($talk_aboutdata['tid']?$talk_aboutdata['tid']:$talk_aboutdata['id']); ?>" class="tabroadcast" href="javascript:;">
                                <?php echo L("broadcast");?>(<?php echo ($talk_aboutdata["forwarding"]); ?>)
                            </a>
                            <span class="lin">
                                |
                            </span>
                            <a tid="<?php echo ($talk_aboutdata["id"]); ?>" class="comment" href="javascript:;">
                                <?php echo L("comment");?> (
                                <span>
                                    <?php echo ($talk_aboutdata["comment"]); ?>
                                </span>
                                )
                            </a>
                            <span class="lin">
                            ...
                            <a tid="<?php echo ($talk_aboutdata["id"]); ?>" class="likes" href="javascript:;">
                                <?php echo L("like");?>(
                                <span>
                                    <?php echo ($talk_aboutdata["likes"]); ?>
                                </span>
                                )
                            </a>
                        </div>
            ...
            <div class="jvf_frame ss_detail clearfix">
            	<div class="extension_top" id="exchange_tab">
                	<ul class="clearfix">
                    	<li class="selected"><a href="javascript:;"><?php echo L("broadcast");?></a></li>
                        <li><a href="javascript:;"><?php echo L("comment");?></a></li>
                    </ul>
                </div>
                <div id="exchange">
                	<div class="extension_exchange clearfix"></div>
                	<div class="extension_exchange clearfix ss_bgpingl" style="display:none;" tid="<?php echo ($talk_aboutdata["id"]); ?>">
        ...
        <div class="detail_right jvf_fr">
            <!--地图-->
            <?php if(empty($talk_aboutdata["gid"])): ?><div class="mb_head_user mg0a">
  ...
  <div class="mb_username clearfix"><a charset="jvf_fl" href="<?php echo U('User/space/id/'.$userdata['id']);?>"><?php echo ($userdata["name"]); ?>
            ...
            <?php else: ?>
				<div class="shop_all masonry-brick">
				     <div class="jvf_frame w225">
				         <div class="shopall_img"><a href="<?php echo U('Goods/index/id/'.$goodsdata['id']);?>"><img width="213" src="<?php echo ($goodsdata["accessory"]["0"]["path"]); ?>"></a></div>
				         <div class="shop_name"><a href="<?php echo U('Goods/index/id/'.$goodsdata['id']);?>"><?php echo ($goodsdata["title"]); ?></a></div>
				         <div class="shop_adr clearfix"><a class="jvf_fl jvf_over mw145 jvf_address" href="javascript:;" gid="<?php echo ($goodsdata["id"]); ?>"><?php echo ($goodsdata["address"]); ?></a><span class="jvf_juli fujin_ico"><?php echo distance($locate['lat'],$locate['lng'],$goodsdata['latitude'],$goodsdata['longitude']);?></span></div>
				         <div class="shop_conbot clearfix">
				         	<a href="<?php echo U('Goods/index/id/'.$goodsdata['id']);?>"><?php echo L("detail_text");?></a>
				             <a href="<?php echo U('Goods/index/id/'.$goodsdata['id']);?>"><?php echo L("recommend");?>(<?php echo ($goodsdata["recommend"]); ?>)</a>
				             <a href="<?php echo U('Goods/index/id/'.$goodsdata['id']);?>"><?php echo L("evaluate");?>(<?php echo ($goodsdata["comment"]); ?>)</a>
				             <?php if(in_array($goodsdata['id'],$shoppingCartId)): ?><a class="jvf_ico cartHover icow" href="<?php echo U('Goods/updateNum/id/'.$goodsdata['id'].'/num/0');?>" rel="<?php echo ($goodsdata['id']); ?>"></a>
				             <?php else: ?>
				             <a class="jvf_ico cart icow" href="<?php echo U('Goods/buy/id/'.$goodsdata['id']);?>" rel="<?php echo ($goodsdata['id']); ?>"></a><?php endif; ?>
				             <?php if(in_array($goodsdata['id'],$collection_arr)): ?><a class="jvf_ico collHover icow" href="<?php echo U('Member/removeFavorites/gid/'.$goodsdata['id']);?>"></a>
				             <?php else: ?>
				             <a class="jvf_ico coll icow" href="<?php echo U('Member/saveFavorites/gid/'.$goodsdata['id']);?>"></a><?php endif; ?>
				         </div>
				         <div class="jvf_frame_bot clearfix">
				         	<div class="jvf_fl">
				         	<a href="<?php echo U('User/space/id/'.$goodsdata['promulgator']['id']);?>" uid="<?php echo ($goodsdata['promulgator']['id']); ?>" class="jvf_getUser"><img width="35" height="35" src="<?php echo ($goodsdata["promulgator"]["header"]["thumbnail"]); ?>"></a>
				             </div>
				             <div class="shop_zur jvf_fl clearfix">
				             	<span class="jvf_fl"><?php echo L("cost_price");?>：</span><span class="yuanjia">&yen;<?php echo ($goodsdata["original"]); ?></span>
				                 <div class="jvf_cl"></div>
				                 <span class="jvf_fl"><?php echo L("goods_manager");?>：</span><span class="jvf_fl jvf_over w100 user_name"><a href="<?php echo U('User/space/id/'.$goodsdata['promulgator']['id']);?>"><?php echo ($goodsdata["promulgator"]["name"]); ?></a></span>
				             </div>
				             <div class="jvf_xianjia">&yen;<?php echo ($goodsdata["price"]); ?></div>
				         </div>
