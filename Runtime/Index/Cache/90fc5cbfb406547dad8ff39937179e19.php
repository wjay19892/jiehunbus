...
<div class="mainbody body_bot body_con clearfix">
	<div class="jvf_member_body clearfix">
       <div class="mb_left jvf_fl3">
       		<!--资料-->
            <h2 class="jvf_space_tit"><?php echo L("ta_information");?></h2>
            <div class="jvf_info">
            	<div class="jvf_info_tit"><?php echo L("basic_info");?></div>
                <div class="jvf_info_con">
                    <ul class="clearfix">
                        <li><label><?php echo L("username_text");?>：</label><?php echo ($userdata["name"]); ?></li>
                        <li><label><?php echo L("level_text");?>：</label><?php echo ($userdata["level"]["name"]); ?></li>
                        <li class="meber_name"><label><?php echo L("gender");?>：<?php if(($userdata["sex"])  ==  "0"): ?><span class="jvf_ico unman linebl vmid"></span><?php endif; ?>
							       	<?php if(($userdata["sex"])  ==  "1"): ?><span class="jvf_ico man linebl vmid"></span><?php endif; ?>
							       	<?php if(($userdata["sex"])  ==  "2"): ?><span class="jvf_ico woman linebl vmid"></span><?php endif; ?>
       					</li>
                        <li><label><?php echo L("mail_text");?>：</label><?php echo ($userdata["mail"]); ?></li>
                        <li><label><?php echo L("reg_time");?>：</label><?php echo (toDateRefer($userdata["regtime"])); ?></li>
                        <li><label><?php echo L("goods_index_last_login_time");?>：</label><?php echo (dgmdate($userdata["lastlog"]["addtime"])); ?></li>
                    </ul>
                    <p><label><?php echo L("homeland");?>：</label><?php echo ($userdata["address"]); ?></p>
                    <p><label><?php echo L("common_position");?>：</label><?php echo ($userdata["location"]["address"]); ?></p>
                    <p><label><?php echo L("personalized_signature");?>：</label><?php echo ($userdata["self_introduction"]); ?></p>
                </div>
                <div class="jvf_info_tit"><?php echo L("member_edit_help");?></div>
                <div class="jvf_info_con">
                	<ul class="clearfix" style="border-bottom:none;">
                        <?php if(is_array($mattlist)): $i = 0; $__LIST__ = $mattlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$att): ++$i;$mod = ($i % 2 )?><li class="bder_topno">
			                    <label for="user_att<?php echo ($att["id"]); ?>"><?php echo ($att["key"]); ?>：</label>
			                    <?php if($att['type'] == 0): ?><!-- 手动输入 -->
			                        <?php if(empty($att["val"])): ?><span><?php echo ($att["default"]); ?></span>
			                        <?php else: ?>
			                        <span><?php echo ($att["val"]); ?></span><?php endif; ?><?php endif; ?>
			                    
			                    <?php if($att['type'] == 1): ?><?php if(empty($att["val"])): ?><span><?php echo ($att["default"]); ?></span>
			                        <?php else: ?>
			                        <span><?php echo ($att["val"]); ?></span><?php endif; ?><?php endif; ?>
			                    
			                    <?php if($att['type'] == 2): ?><?php if(empty($att["val"])): ?><span><?php echo ($att["default"]); ?></span>
			                        <?php else: ?>
			                        <span><?php echo ($att["val"]); ?></span><?php endif; ?><?php endif; ?>
			                    ...
			                    <?php if($att['type'] == 4): ?><?php if(empty($att["val"])): ?><span><?php echo ($att["default"]); ?></span>
		                        <?php else: ?>
		                        <?php if(is_array($att["val"])): $i = 0; $__LIST__ = $att["val"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): ++$i;$mod = ($i % 2 )?><span><?php echo ($val); ?></span><?php endforeach; endif; else: echo "" ;endif; ?><?php endif; ?><?php endif; ?>     
			                                                   
			                    <?php if($att['type'] == 5): ?><!-- 图片域 -->
			                        <?php if(empty($att["val"])): ?><span><?php echo ($att["default"]); ?></span>
			                        <?php else: ?>
			                        <img src="__ROOT__<?php echo ($att["val"]); ?>"><?php endif; ?><?php endif; ?>
                ...
                <div class="jvf_info_tit"><?php echo L("user_quanzi");?></div>
                <div class="user_quanzi">
                ...
                <div class="jvf_info_tit"><?php echo L("user_biaoqian");?></div>
