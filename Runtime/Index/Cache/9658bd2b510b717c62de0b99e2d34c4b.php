...
<div class="detail_bot jvf_allimg"></div>
 <!--发表感受-->
      			<div class="extension_top">
                	<ul class="clearfix">
                    	<li class="selected"><a href="javascript:;"><?php echo L("inbox_text");?></a></li>
                    </ul>
                </div>
                <div class="clearfix shaix">
                <form method="get" id="inbox_filter_form" action="<?php echo U('Member/inbox');?>">
                		<div class="jvf_fl" style="margin-top:3px;"><?php echo L("member_show");?>：
                           <select id="filter" name="filter" onchange="jQuery('form#inbox_filter_form').submit();">
                                <option <?php if(($filter == "") OR ($filter == "all")): ?>selected="selected"<?php endif; ?> value="all"><?php echo L("member_inbox_all");?>  (<?php echo ($mebcount); ?>)</option>
                    <option <?php if(($filter == "site")): ?>selected="selected"<?php endif; ?> value="site"><?php echo L("member_inbox_site");?> (<?php echo ($sitecount); ?>)</option>
                    <option <?php if(($filter == "friends")): ?>selected="selected"<?php endif; ?> value="friends"><?php echo L("member_inbox_friends");?> (<?php echo ($friendscount); ?>)</option>
                    <option <?php if(($filter == "unread")): ?>selected="selected"<?php endif; ?> value="unread"><?php echo L("member_inbox_unread");?> (<?php echo ($unreadcount); ?>)</option>
                            </select>
                        </div>
                 </form>
                        <div class="jvf_fr">
                            <a class="btn linebl f14 p2153" style="margin-left:10px;" id="sendpm" href="<?php echo U('Member/sendpm');?>"><?php echo L("send_message");?></a>
                        </div>
                     </div>
                <div class="middle clearfix">
                	
                    	<ul id="inbox_box">
                    	<?php if(is_array($mebmessages)): $i = 0; $__LIST__ = $mebmessages;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li class="clearfix">
                        		<a class="jvf_fl jvf_over detail" href="<?php echo U('Member/viewpm/id/'.$vo['id']);?>" title="<?php echo L("member_viewpm");?>"><?php echo (msubstr($vo["content"],0,30)); ?></a>
            					<span class="jvf_fr">
            						<a href="<?php echo U('User/space/id/'.$vo['send']);?>" title="<?php echo L("view_profile");?>"><?php echo ($vo["sendname"]); ?></a>
            						&nbsp;<?php echo L("send_in");?>&nbsp;
            						<span title="<?php echo (toDate($vo["addtime"])); ?>"><?php echo (dgmdate($vo["addtime"])); ?></span>
            						&nbsp;&nbsp;
            						<a href="<?php echo U('Member/replypm/id/'.$vo['id']);?>" title="<?php echo L("reply");?>" class="reply"><?php echo L("reply");?></a>
            						&nbsp;&nbsp;
            						<a href="<?php echo U('Member/deletepm/id/'.$vo['id']);?>" title="<?php echo L("delete");?>" class="delete"><?php echo L("delete");?></a>
            					</span>
