   ...
   <!-- 功能操作区域结束 -->
	<table class="table" width="100%" layoutH="138">
		...
		<tr>
			...
			<th width="200" orderField="cash" <?php if($_REQUEST["_order"] == 'cash'): ?>class="<?php echo ($_REQUEST["_sort"]); ?>"<?php endif; ?>>提现金额</th>
			<th width="200" orderField="bank_id" <?php if($_REQUEST["_order"] == 'bank_id'): ?>class="<?php echo ($_REQUEST["_sort"]); ?>"<?php endif; ?>>提现方式</th>
			<th width="200" orderField="bank_card" <?php if($_REQUEST["_order"] == 'bank_card'): ?>class="<?php echo ($_REQUEST["_sort"]); ?>"<?php endif; ?>>账户</th>
			<th width="200" orderField="addtime" <?php if($_REQUEST["_order"] == 'addtime'): ?>class="<?php echo ($_REQUEST["_sort"]); ?>"<?php endif; ?>>提现时间</th>
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><tr target="sid_node" rel="<?php echo ($vo['id']); ?>">
				...
				<td><?php echo (getWithdraw($vo['status'])); ?></td>
				<td>
				<a href="__URL__/edit/id/<?php echo ($vo['id']); ?>" target="dialog" rel="__MODULE___edit">编辑</a>
				<?php if($vo['status'] == 0): ?><a href="__URL__/handle/id/<?php echo ($vo['id']); ?>/navTabId/__MODULE__" target="ajaxTodo" calback="navTabAjaxMenu" title="你确定要设为已处理？" warn="请选择">处理</a>
				<a href="__URL__/revocation/id/<?php echo ($vo['id']); ?>/navTabId/__MODULE__" target="ajaxTodo" calback="navTabAjaxMenu" title="你确定要设为已撤销？" warn="请选择">撤销</a>
				<?php elseif($vo['status'] == 1): ?>
				<a href="__URL__/complete/id/<?php echo ($vo['id']); ?>/navTabId/__MODULE__" target="ajaxTodo" calback="navTabAjaxMenu" title="你确定要设为已完成？" warn="请选择">完成</a>
				<a href="__URL__/revocation/id/<?php echo ($vo['id']); ?>/navTabId/__MODULE__" target="ajaxTodo" calback="navTabAjaxMenu" title="你确定要设为已撤销？" warn="请选择">撤销</a><?php endif; ?>
				</td>
