...
class AccessoryModel extends CommonModel {
	public function getOne($id){
		$data = $this->find($id);
		if(!empty($data)){
			$data['origin'] = __ROOT__.$data['origin'];
			$data['path'] = __ROOT__.$data['path'];
			$data['thumbnail'] = __ROOT__.$data['thumbnail'];
		}
		return $data;
	}
	//图片上传的基本方法。未完成。待配置扩展......
		 ...
		 if (!empty($_FILES)) {
	        ...
	        if (!$upload->upload()) {
				...
				foreach($uploadList as $k=>$fileItem){
					if($water&&file_exists($sysconfig['site_water_image'])&&$sysconfig['site_water_mark'] == 1){
						import("ORG.Util.Image");
						$path = $fileItem['savepath'].'m_'.$fileItem['savename'];							
						Image::water($path,$sysconfig['site_water_image'],"",$sysconfig['site_water_alpha'],$sysconfig['site_water_position']);	
					}
	...
	//不生成缩略图的图片上传办法
	public function uploadImg($water = 0,$dir='attachment',$showstatus = false){
		 $sysconfig = C('sysconfig');
		 if (!empty($_FILES)) {
		 	import("ORG.Net.UploadFile");
        	$upload = new UploadFile();
	        ...
	        if (!$upload->upload()) {
				...
				foreach($uploadList as $k=>$fileItem){
					if($water&&file_exists($sysconfig['site_water_image'])&&$sysconfig['site_water_mark'] == 1){
						import("ORG.Util.Image");
						$path = $fileItem['savepath'].$fileItem['savename'];							
						Image::water($path,$sysconfig['site_water_image'],"",$sysconfig['site_water_alpha'],$sysconfig['site_water_position']);	
					}
					$recpath = str_replace(ROOT_PATH,"",$upload->savePath);
					$data['type'] = 'img';
